...
class ResNet(nn.Module):
    def __init__(
        ...
        else: # for input size 224x224
            self.conv1 = wrapped_conv(224, 3, 64, kernel_size=3, stride=2)
            self.layer1 = self._make_layer(block, 56, 64, num_blocks[0], stride=1)
            self.layer2 = self._make_layer(block, 56, 128, num_blocks[1], stride=2)
            self.layer3 = self._make_layer(block, 28, 256, num_blocks[2], stride=2)
            self.layer4 = self._make_layer(block, 14, 512, num_blocks[3], stride=2)
    ...
    def forward(self, x):
        out = self.activation(self.bn1(self.conv1(x)))
        out = self.maxpool(out) # for input size 224
        out = self.layer1(out)
